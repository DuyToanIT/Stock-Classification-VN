...
def process(pathData):
    ...
    label1 = np.array(processKmeans('./Result/NotPreProcessData/Low_Low',df,2, arr))
    ...
    label2 = np.array(processKmeans('./Result/NotPreProcessData/Close_Close',df,2, arr2))
    plt.subplot(222)
    plt.scatter(arr2[:,0],arr2[:,1], c= label2)
    plt.title('Close-Close')

    arr3 = np.array(df.loc[:,['<CloseFixed>','<LowFixed>']])
    label3 = np.array(processKmeans('./Result/NotPreProcessData/Close_Low',df,2, arr3))
    plt.subplot(223)
    plt.scatter(arr3[:,0],arr3[:,1], c= label3)
    plt.title('Close-Low')

    arr4 = np.array(df.loc[:,['<LowFixed>','<CloseFixed>']])
    label4 = np.array(processKmeans('./Result/NotPreProcessData/Low_Close',df,2, arr4))
    plt.subplot(224)
